...
import { HackMetadata, newServerStats } from "./hacksMetadata";
...
export function collectCracks(ns: NS, metadata: Metadata) {
  ...
  for (const crack of Object.keys(CrackTypeToFile)) {
    if (
      !metadata.cracks[crack] &&
      ns.fileExists(CrackTypeToFile[crack], "home")
    ) {
...
export function crackNPCServer(
  ns: NS,
  metadata: Metadata,
  server: string
): boolean {
...
export function nukeServers(
  ...
  for (const server of metadata.newServers) {
    const isPlayerServer = server.startsWith(PlayerServerPrefix);
    if (
