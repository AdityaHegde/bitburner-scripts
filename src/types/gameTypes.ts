...
export interface CodingContract {
   ...
   * @param server - Host of the server containing the contract. Optional. Defaults to current server if not provided.
   * @param opts - Optional parameters for configuring function behavior.
   * @returns True if the solution was correct, false otherwise. If the returnReward option is configured, then the function will instead return a string. If the contract is successfully solved, the string will contain a description of the contract’s reward. Otherwise, it will be an empty string.
   */
  attempt(answer: string[] | number, filename: string, server?: string, opts?: CodingAttemptOptions): boolean | string;
   ...
   */
  getContractType(filename: string, server?: string): string;
   ...
   */
  getDescription(filename: string, server?: string): string;
   ...
   */
  getData(filename: string, server?: string): any;
   ...
   */
  getNumTriesRemaining(filename: string, server?: string): number;
}
...
export interface NS extends Singularity {
   ...
   * ```
   * @param server - Hostname of the target server to hack.
   * @param opts - Optional parameters for configuring function behavior.
   * @returns The amount of money stolen if the hack is successful, and zero otherwise.
   */
  hack(server: string, opts?: BasicHGWOptions): Promise<number>;
   ...
   * ```
   * @param server - Hostname of the target server to grow.
   * @param opts - Optional parameters for configuring function behavior.
   * @returns The number by which the money on the server was multiplied for the growth.
   */
  grow(server: string, opts?: BasicHGWOptions): Promise<number>;
   ...
   * ```
   * @param server - Hostname of the target server to weaken.
   * @param opts - Optional parameters for configuring function behavior.
   ...
   */
  weaken(server: string, opts?: BasicHGWOptions): Promise<number>;
   ...
   * ```
   * @param server - Hostname of the target server to analyze.
   * @param hackAmount - Amount of money you want to hack from the server.
   * @returns The number of threads needed to hack the server for hackAmount money.
   */
  hackAnalyzeThreads(server: string, hackAmount: number): number;
   ...
   */
  hackAnalyzeChance(server: string): number;
   ...
   */
  growthAnalyze(server: string, growthAmount: number, cores?: number): number;
   ...
   * @param server - Optional. Hostname of the server that the script is on.
   ...
   */
  getScriptLogs(fn?: string, server?: string, ...args: any[]): string[];
   ...
   * @param server - Optional. Hostname of the script being tailed. Defaults to the server this script is running on. If args are specified, this is not optional.
   * @param args - Arguments for the script being tailed.
   */
  tail(fn?: FilenameOrPID, server?: string, ...args: any[]): void;
   ...
   * array are strings.
   *
   * @param server - Optional, Hostname of the server to scan, default to current server.
   * @returns Returns an string of hostnames.
   */
  scan(server?: string): string[];
   ...
   * @param server - Hostname of the `target server` on which to execute the script.
   * @param numThreads - Optional thread count for new script. Set to 1 by default. Will be rounded to nearest integer.
   * @param args - Additional arguments to pass into the new script that is being run. Note that if any arguments are being passed into the new script, then the third argument numThreads must be filled in with a value.
   * @returns Returns the PID of a successfully started script, and 0 otherwise.
   */
  exec(script: string, server: string, numThreads?: number, ...args: Array<string | number | boolean>): number;
   ...
   * @param server - Hostname of the server on which to kill the script.
   ...
   * ```
   */
  kill(script: string, server: string, ...args: string[]): boolean;
   ...
   * If no server is defined, it will kill all scripts, where the script is running.
   *
   * @param server - IP or hostname of the server on which to kill all scripts.
   * @returns True if any scripts were killed, and false otherwise.
   */
  killall(server?: string): boolean;
   ...
   *
   * @param server - Host of the target server.
   ...
   */
  ls(server: string, grep?: string): string[];
   ...
   * }
   * ```
   * @param server - Host address of the target server. If not specified, it will be the current server’s IP by default.
   * @returns Array with general information about all scripts running on the specified target server.
   */
  ps(server?: string): ProcessInfo[];
   ...
   */
  hasRootAccess(server: string): boolean;
  ...
  /**
   * Returns a server object for the given server. Defaults to the running script's server if server is not specified.
   *
   * @remarks
   * RAM cost: 2 GB
   * @param server - Optional. Hostname for the requested server object.
   ...
   * ```
   * @param server - Host of target server
   * @returns Amount of money available on the server.
   */
  getServerMoneyAvailable(server: string): number;
   ...
   */
  getServerMaxMoney(server: string): number;
   ...
   */
  getServerGrowth(server: string): number;
   ...
   */
  getServerSecurityLevel(server: string): number;
   ...
   */
  getServerMinSecurityLevel(server: string): number;
   ...
   */
  getServerBaseSecurityLevel(server: string): number;
   ...
   */
  getServerRam(server: string): [number, number];
   ...
   * @returns max ram (GB)
   */
  getServerMaxRam(server: string): number;
  /**
   ...
   * @returns used ram (GB)
   */
  getServerUsedRam(server: string): number;
   ...
   * @returns The required hacking level of the target server.
   */
  getServerRequiredHackingLevel(server: string): number;
   ...
   * @returns The number of open ports required to successfully run NUKE.exe on the specified server.
   */
  getServerNumPortsRequired(server: string): number;
   ...
   */
  serverExists(server: string): boolean;
   ...
   * @param server - Host of target server. This is optional. If it is not specified then the function will use the current server as the target server.
   ...
   */
  isRunning(script: FilenameOrPID, server: string, ...args: string[]): boolean;
   ...
   * @param hostname - Optional. Name of server server the script is running on.
   ...
   *
   * @param server - Host of the server to delete.
   * @returns True if successful, and false otherwise.
   */
  deleteServer(server: string): boolean;
   ...
   * @param server - Host Address of the server on which to delete the file. Optional. Defaults to current server.
   * @returns True if it successfully deletes the file, and false otherwise.
   */
  rm(name: string, server?: string): boolean;
   ...
   */
  scriptRunning(script: string, server: string): boolean;
   ...
   */
  scriptKill(script: string, server: string): boolean;
   ...
   * @param server - Host of target server the script is located on. This is optional, If it is not specified then the function will se the current server as the target server.
   * @returns Amount of RAM (in GB) required to run the specified script on the target server, and 0 if the script does not exist.
   */
  getScriptRam(script: string, server?: string): number;
   ...
   * @param server - Server on which script is running.
   ...
   */
  getScriptIncome(script: string, server: string, ...args: string[]): number;
   ...
   */
  getScriptExpGain(script: string, server: string, ...args: string[]): number;
   ...
   * @param server - Optional hostname/ip of server for target file.
   * @returns True if the data was successfully retrieved from the URL, false otherwise.
   */
  wget(url: string, target: string, server?: string): Promise<boolean>;
   ...
   */
  mv(server: string, source: string, destination: string): void;
